...
namespace OpenLoco::GameSaveCompare
{
    ...
    static bool compareGameStates(S5::GameState& gameState1, S5::GameState& gameState2, bool displayAllDivergences)
    {
        ...
        foundDivergence |= isLoggedDivergentGameStateField("defaultTreeObjectId", 0, gameState1.general.defaultTreeObjectId, gameState2.general.defaultTreeObjectId);
        ...
        foundDivergence |= isLoggedDivergentGameStateField("defaultLandObjectId", 0, gameState1.general.defaultLandObjectId, gameState2.general.defaultLandObjectId);
        ...
        foundDivergence |= isLoggedDivergentGameStateField("defaultMiscBuildingObjectId", 0, gameState1.general.defaultMiscBuildingObjectId, gameState2.general.defaultMiscBuildingObjectId);
        foundDivergence |= isLoggedDivergentGameStateField("defaultWallObjectId", 0, gameState1.general.defaultWallObjectId, gameState2.general.defaultWallObjectId);
            ...
            for (auto x = 0; x < 384; ++x)
            {
                ...
                for (; i < limit; ++i)
                {
                    if (!bitWiseEqual(t1s[i], t2s[i]))
                    {
                        ...
                        {
                            Logging::info("TILE ELEMENT[{}] TYPE[{}] x:{}, y:{}", elementCount, static_cast<int>(enumValue(t1s[i].type())), x, y);
                        }
